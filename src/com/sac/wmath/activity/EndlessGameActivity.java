 ...
 * 
 */
public class EndlessGameActivity extends Activity implements CountDownListener, View.OnClickListener {
    ...
    private void init() {
        ...
        for (int offset = 0; offset < mJudgeNumber; offset++) {
            mAnswerList.add(Operation.createOperation(Constants.OPERATION_TYPE_JUDGE,1));
        }
        Random r = new Random();
        for (int offset = 0; offset < mCalculateNumber; offset++) {
            int type = r.nextInt(4);
            if (type == 0) {
                mAnswerList.add(Operation.createOperation(Constants.OPERATION_TYPE_FILL_OPERAND_LEFT,1));
            } else if (type == 1) {
                mAnswerList.add(Operation.createOperation(Constants.OPERATION_TYPE_FILL_OPERAND_RIGHT,1));
            } else if (type == 2) {
                mAnswerList.add(Operation.createOperation(Constants.OPERATION_TYPE_FILL_CALCULATE_OPERATOR,1));
            } else {
                mAnswerList.add(Operation.createOperation(Constants.OPERATION_TYPE_FILL_RESULT,1));
            }
        }
    }
    ...
    private void initView() {
        ...
        //隐藏pass error
        mPassShowView.setVisibility(View.INVISIBLE);
        mErrorShowView.setVisibility(View.INVISIBLE);
    ...
    private synchronized void next() {
        ...
        if (mPassNumber > 0 || mErrorNumber > 0) {
            Intent intent = new Intent(this, EndlessModeResultActivity.class);
